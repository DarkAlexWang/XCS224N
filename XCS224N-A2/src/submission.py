...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  # center_word_vec:  [embedding_dimension,1]
  # outside_vectors: [vocab_size,embedding_dimension]
  # (vocab_size,1)
  ...
  dscores = probs.copy()
  #  y_hat minus y
  dscores[outside_word_idx] = dscores[outside_word_idx] - 1
  # (embedding_dim,1)
  grad_center_vec = np.matmul(outside_vectors.T, dscores)
  # print(dscores.shape) # (5,)
  # print(center_word_vec.shape) # (3,)
  # exit()
  grad_outside_vecs = np.outer(dscores, center_word_vec)

  ## another implementation
  ### (d, ) -> (d, 1)
  ##center_word_vec = center_word_vec[:, np.newaxis]
  ##y_hat = softmax(np.matmul(outside_vectors, center_word_vec).T).T
  ###print(y_hat, y_hat[outside_word_idx])
  ##loss = -np.log(float(y_hat[outside_word_idx]))

  ##y = np.zeros_like(y_hat)
  ##y[outside_word_idx] = 1
  ##grad_center_vec = np.matmul(outside_vectors.T, y_hat - y)
  ##grad_outside_vecs = np.matmul(y_hat - y, center_word_vec.T)
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  outside_word_idx_list = [word2ind[i] for i in outside_words]
  for outside_word_idx in outside_word_idx_list:
    word_loss, word_grad_center_vec, word_grad_outside_vec = word2vec_loss_and_gradient(
          center_word_vec, outside_word_idx, outside_vectors, dataset
      )
    loss += word_loss
    grad_center_vecs[center_word_idx] += word_grad_center_vec.reshape(-1)
    grad_outside_vectors += word_grad_outside_vec
