...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        #self.embed_to_hidden = nn.Parameter(torch.empty(self.n_features * self.embed_size, self.hidden_size))
        #nn.init.xavier_uniform_(self.embed_to_hidden)
        #self.embed_to_hidden = nn.Parameter(torch.empty(self.hidden_size))
        #nn.init.uniform_(self.embed_to_hidden)
        #self.dropout = nn.Dropout(self.dropout_prob)
        #self.hidden_to_logits = nn.Parameter(torch.empty(self.hidden_size, self.n_classes))
        #nn.init.xavier_uniform_(self.hidden_to_logits)
        #self.hidden_to_logits = nn.Parameter(torch.empty(self.n_classes))
        #nn.init.uniform_(self.hidden_to_logits)
        ### END CODE HERE
    ...
    def embedding_lookup(self, t):
        ...
        # Second way of implementation
        #x = torch.index_select(self.pretrained_embeddings, 0, torch.flatten(t)).view(t.size(0) , -1)
    ...
    def forward(self, t):
        ...
        #hidden_state = F.relu(torch.matmul(embedding, self.embed_to_hidden) + self.embed_to_hidden)
        #hidden_dropout = self.dropout(hidden_state)
        #logits = torch.matmul(hidden_dropout, self.hidden_to_logits) + self.hidden_to_logits

        embedding = self.embedding_lookup(t)
        embed_hidden = self.embed_to_hidden(embedding)
        hidden_state = nn.functional.relu(embed_hidden)
        hidden_dropout = self.dropout(hidden_state)
