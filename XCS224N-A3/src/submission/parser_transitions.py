class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        """
        ### START CODE HERE
        # Push and pop the stack simultaneously
        if transition == 'S':
            self.stack.append(self.buffer.pop(0))
        # Append the transition arc and pop the dependent (second item) off the stack.
        elif transition == 'LA':
            self.dependencies.append((self.stack[-1], self.stack.pop(-2)))
        # Append the transition arc and pop the first item from the stack
        else:
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # Create a shallow copy of partial_parses
    unfinished_parses = partial_parses[:]
    # Stop when all parses are finished
    while unfinished_parses:
        # Predict for each partial parse in the minibatch.
        ...
        for parse, transition in zip(unfinished_parses[:batch_size], transitions):
            ...
            # Empty buffer and stack of size 1
